...
namespace kiko
{
	...
	{
	public:
		using event_func = std::function<void(const Event&)>;
		struct EventDispatcher
		{
			IEventListener* listener = nullptr; // object that can listen for events
			event_func func; // function that is called on event
		};
		void Update(float dt);

		void Subscribe(Event::id_t id, IEventListener* listener, event_func function);
